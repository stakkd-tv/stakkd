class GuestStarsController < ApplicationController
  ...
  def index
    @table_presenter = Tabulator::CastMembersPresenter.new(@episode.guest_stars.includes(:person))
  end

  def create
    @cast_member = @episode.guest_stars.new(cast_member_params)
    if @cast_member.save
      redirect_to show_season_episode_guest_stars_path(@episode, season_id: @season, show_id: @show)
    else
      redirect_to show_season_episode_guest_stars_path(@episode, season_id: @season, show_id: @show), alert: "Could not add that cast member."
    end
  end
  ...
  def set_episode
    @episode = @season.episodes.find_by!(number: params.expect(:episode_id))
  end

  def set_cast_member
    @cast_member = @episode.guest_stars.find(params.expect(:id))
  end
end
