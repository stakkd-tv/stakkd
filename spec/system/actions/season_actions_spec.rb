...
RSpec.feature "Season actions", type: :system, js: true do
  ...
  def remove_from_history(season = nil, expected_history_count: 0)
    ...
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Season'][data-record-id='#{season.id}'][data-status='not_watched']"
  ...
  scenario "adding to and removing from history from the season page" do
    ...
    # Episode add to history is shown, in an unwatched state
    ...
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Episode'][data-record-id='#{unreleased_episode.id}'][data-status='not_watched'][disabled]"
    ...
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Season'][data-record-id='#{@season.id}'][data-status='watched']"
    ...
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Episode'][data-record-id='#{unreleased_episode.id}'][data-status='not_watched'][disabled]"
    remove_from_history
    # Episode add to history button state is updated, now unwatched
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Episode'][data-record-id='#{episode.id}'][data-status='not_watched']"
    # Unreleased episode does not have its state updated
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Episode'][data-record-id='#{unreleased_episode.id}'][data-status='not_watched'][disabled]"
  ...
  scenario "add to and removing from history from the show page" do
    ...
    special_episode = FactoryBot.create(:episode, season: specials, number: 1, original_air_date: Date.today)
    ...
    # Show is not watched at all
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Show'][data-record-id='#{@season.show.id}'][data-status='not_watched']"
    ...
    expect(@user.history_items.first.item).to eq episode
    expect(@user.history_items.first.consumed_at).to eq Time.current
    # The show is now partially watched
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Show'][data-record-id='#{@season.show.id}'][data-status='partially_watched']"
    ...
    expect(page).not_to have_css "dialog[data-controller='history-dialog']"
    # Add to history button state is updated
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Season'][data-record-id='#{season2.id}'][data-status='watched']"
    ...
    expect(@user.history_items.first.consumed_at).to eq Time.current
    # The show is now fully watched as season 3 is not yet released and specials don't count
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Show'][data-record-id='#{@season.show.id}'][data-status='watched']"

    # Removing season 1 from history
    remove_from_history(expected_history_count: 1) # Season 2 is still watched, history count is 1
    # Show is now partially watched as only season 2 is watched
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Show'][data-record-id='#{@season.show.id}'][data-status='partially_watched']"

    # Removing season 2 from history
    remove_from_history(season2)
    # Show is now not watched as no episodes are watched
    expect(page).to have_css "button[data-controller='history-button'][data-record-type='Show'][data-record-id='#{@season.show.id}'][data-status='not_watched']"
  end
end
